...
: NCFFNAME. ( -- )
  ...
  SETXY FILEINFO ZCOUNT CUR_DIR C@ /STRING
...
: NC_SETNAME  ( n -- )
  ...
  NC_CUR = 
  DUP	IF  FILEINFO ZCOUNT COUNT_NAME $!
	    FILEINFO IDF2TYPE '5' = TO ?COUNT_DIR
	THEN
 NCSETCOLOR AND
  NC_COLOR  XOR 
  FILEINFO IDF2TYPE '5' =  IF 0x4 XOR THEN
 COLOR! ;
...
\  S" "  OPEN-FILE DUP IF S" ./" $CHDIR ( ." CD" KEY DROP) THEN
\  THROW TO DIR_ID
    INITRDADR TO FILEINFO

    NC_X_BASE 1+ NC_Y_BASE 1+ SETXY

   NC_BASE 0 ?DO NEXTFILE DROP LOOP
...
\+ STS.	$13	OF 2>R >R STS. R> 2R> ENDOF
  ...
  2R> NC_ROWS 2+ UMAX SETXY  R>  COLOR!  ENDOF  \ tab
	...
	0xA	OF   >R OVER R>  TUCK U< IF -1 BREAK
		  ?COUNT_DIR
		  IF	COUNT_NAME COUNT
			2DUP CUR_DIR@ COMPARE 0=
			IF 1- BEGIN 1- 2DUP + C@ '/' = UNTIL 1+ THEN
			$CHDIR
...
: NC \ TOGGG
 ...
 LASTSTP: DIR_DRAW
 LASTSTP: FMTABDO
 LASTSTP: NEXTFILE FILEINFO ZCOUNT TYPE
 LASTSTP: FILEINFO ZCOUNT TYPE
 LASTSTP: NEXTFILE
 LASTSTP: INITRDADR TO FILEINFO
