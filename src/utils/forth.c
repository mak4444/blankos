...
void OrStore(){  *(Cell*)Tos |= *Stack++; Tos = *Stack++;  } pp(OrStore)
void AndStore(){ *(Cell*)Tos &= *Stack++; Tos = *Stack++;  } pp(AndStore)
void XorStore(){ *(Cell*)Tos ^= *Stack++; Tos = *Stack++;  } pp(XorStore)
...
void Emit() {
	colorputc((char)Tos,fg4,bg4);
 Tos = *Stack++; } pp(Emit)

void Space() {colorputc(' ',fg4,bg4); } pp(Space)

void Cr() { putc('\n'); } pp(Cr)

void Type() {
  char* str = (char*)*Stack++;
  while (Tos--) colorputc(*str++, fg4, bg4);
  Tos =  *Stack++;
} pp(Type)

void ZType() { colorputs((char*)Tos, fg4, bg4); Tos = *Stack++;} pp(ZType)

void SetXY() // ( x y -- )
{	move_cursor(*Stack++,Tos);
	Tos = *Stack++;
} pp(SetXY)
...
*/
static void udiv(uint32_t a,uint32_t *b,uint32_t *c)
{
 uint32_t d,qh,ql;
 int i,cy;
 qh=*b;ql=*c;d=a;
...
void ParseName() {
    ...
    *--Stack = Tos;
    while (  addr<Eaddr ) { if( *(uint8_t*)addr > ' ') break;
        addr++; }
    *--Stack=Waddr=addr;
    *v2in = addr - tib[1];
    while ( addr<=Eaddr ) { (*v2in)++; if( *(uint8_t*)addr <= ' ') break;
     addr++; }
    Tos=addr-Waddr;
} pp(ParseName)
...
void Compare(){ 
  ...
  while (len1-- > 0) if(Tos=(*s1++ - *s2++)) { return;}
...
void StrEqu(){ 
  ...
  while (Tos-- > 0) if(*s1++ != *s2++) {Tos=0; return;}
 } pp(StrEqu)
...
void openFile() {
	...
	filename[uu] = 0;
	while(uu--) filename[uu] = caddr[uu];

	header = (tar_header_t*)initrd_addr;

	while (header->filename[0] != '\0')
	{
		if (strcmp(header->filename,&filename) == 0)
		{
		  *(uint32_t*)(&header->devminor[0])=0; // used as the current position
		  *Stack = (sCell)header;  Tos = 0;
		   return;
		}
...
void closeFile() { Tos = 0; } pp(closeFile)
...
void readFile() {
    ...
    tar_header_t *header = (tar_header_t*)Tos; Tos=0;
    uint32_t size = tar_parse_size(header->size);
    uint32_t position = *(uint32_t*)&header->devminor;
    if ((position + len) > size) len = size - position;
    if(!len) { *Stack=0; return; }
	
	void* src= (void*)((uint8_t*)header + 512 + position) ;
	memmove(buffer,src,len);
	*Stack=len;
...
void readLine() {
	...
	*Stack=-1;
	...
	if(!len) { Stack[1]=0; *Stack=0; return; }

	char* src= (void*)((uint8_t*)header + 512 + position) ;
	...
	Stack[1]=ii;
	  if(src[ii]=='\n') ii++;
	  if(src[ii]=='\r') ii++;
        *(uint32_t*)&header->devminor+=ii;
	Tos=0;

} pp(readLine)

void  NextFile(void) {
	tar_header_t *header = (tar_header_t*)Tos;
	uint32_t next_file_offset = ((tar_parse_size(header->size)+TAR_BLOCK_SIZE-1)/TAR_BLOCK_SIZE)*TAR_BLOCK_SIZE;
	Tos = (tar_header_t*)((uint8_t*)header + next_file_offset + TAR_BLOCK_SIZE);
} pp(NextFile)
...
void  KeyQ()
{	*--Stack= Tos;
	Tos = keyboard_has_input();
} pp(KeyQ)

void  Key()
{  *--Stack= Tos;
	Tos= keyboard_getchar() ;
} pp(Key)

void  ScanKeyQ()
{ *--Stack= Tos;
	Tos = sc_keyboard_has_input();
} pp(ScanKeyQ)

void ScanKey()
{  *--Stack= Tos;
	Tos=(uint8_t)sc_keyboard_getchar();
} pp(ScanKey)
...
void CursorOn(){ draw_cursor(white);} pp(CursorOn)
void CursorOff(){ erase_cursor();} pp(CursorOff)

void  IDF2Type() // ( fid -- fid+ )
{     Tos = ((tar_header_t *)Tos)->typeflag ;
} pp(IDF2Type)
...
//     tty_printf("MakeImag run\n");
    ...
    FthItem("!",~pStore);
    FthItem("+!",~pAddStore);
    FthItem("AND!",~pAndStore);
    FthItem("OR!",~pOrStore);
    FthItem("XOR!",~pXorStore);
    FthItem("1+!",~pIncr);
    FthItem("0!",~pOff);
    FthItem("OFF",~pOff);
    FthItem("ON",~pOn);
    ...
    FthItem("KEY?1",~pKeyQ);
    sCell PKeyQ = Header("KEY?");  Co2(~pDoDefer,~pKeyQ);
    FthItem("KEY1",~pKey);
    sCell PKey = Header("KEY");  Co2(~pDoDefer,~pKey);


    FthItem("SCANKEY",~pScanKey);
    FthItem("SCANKEY?",~pScanKeyQ);

    Constant("KBDUS",kbdus);

    FthItem("CURSORON",~pCursorOn);
    FthItem("CURSOROFF",~pCursorOff);
...
//    FthItem("OPEN-FILE",~popenFile);
    sCell PopenFile = Header("OPEN-FILE");  Co2(~pDoDefer,~popenFile);

    FthItem("READ-FILE",~preadFile);
    FthItem("READ-LINE",~preadLine);

    FthItem("CLOSE-FILE",~pcloseFile);
    FthItem(">NEXTFILE",~pNextFile);
    Constant("INITRDADR",initrd_addr);
    FthItem("IDF2TYPE",~pIDF2Type);
    ...
    Constant("&COLOR", (sCell)&fg4);
    Constant("&BGCOLOR", (sCell)&bg4);
...
void program_forth(int argc, char* argv[])
{
	int game_speed;
	if (argc < 2)
	{
	...
	HereArea = malloc(sizeof(sCell)* HERE_SIZE1);
	StackArea = malloc(sizeof(sCell)* STACK_SIZE);
	RStackArea = malloc(sizeof(sCell)* RSTACK_SIZE);
